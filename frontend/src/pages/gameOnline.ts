...
function connectToGame(gameId: string): void {
  ...
  const socket = new WebSocket(`ws://localhost:8002/pong/${gameId}`);
    ...
    try {
      ...
      if (data.tipo === 'bienvenida') {
        ...
        if (statusElement) {
          ...
          statusElement.innerHTML = `<p class="text-green-500">🎮 Eres el Jugador ${playerNumber} vs ${modeText}! Usa W/S para mover</p>`;
        }
      }
      ...
      if (data.tipo === 'estado_general') {
        if (statusElement) {
          ...
          if (gameMode === 'pve' && data.estado && data.estado.includes('Esperando jugador')) {
  ...
  if (config) {
    playerCardsContainer.innerHTML = `
      <div class="${config.you.bgColor} rounded-lg p-3">
        <h3 class="text-lg font-bold text-white">📱 Tú (Jugador ${playerNumber})</h3>
    ...
    playerRoleElement.innerHTML = `
      <div class="text-green-400 font-bold">
        ✅ Eres el Jugador ${playerNumber} vs ${config.opponent.type} (Pala ${config.you.color === 'yellow' ? 'Amarilla' : 'Azul'} - Lado ${config.you.side})
      </div>
    `;
    ...
    if (score1Title && score2Title) {
      if (playerNumber === 1) {
        score1Title.innerHTML = '🟡 Jugador 1 (Tú)';
        score2Title.innerHTML = isPvE ? '🔵 IA (Oponente)' : '🔵 Jugador 2 (Oponente)';
      } else {
        score1Title.innerHTML = isPvE ? '🟡 IA (Oponente)' : '🟡 Jugador 1 (Oponente)';
        score2Title.innerHTML = '🔵 Jugador 2 (Tú)';
      }
    }
  }
}
...
function drawInitialCanvas(ctx: CanvasRenderingContext2D): void {
  ...
  // Mensaje de espera
  ctx.fillStyle = 'white';
// Mejorar visualización del marcador
  ctx.font = '24px Arial';
...
function showWinnerModal(message: string, gameState: any): void {
  ...
  if (modal && winnerMessage && finalScore) {
    ...
    finalScore.textContent = `Puntuación Final: ${gameState.puntuacion.jugador1} - ${gameState.puntuacion.jugador2}`;
