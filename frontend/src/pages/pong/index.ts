...
export function renderPongPage(gameMode: GameMode = "1v1_local", aiDifficulty: AIDifficulty = "MEDIUM"): void {
  ...
  // Check if this is an online mode
  const isOnlineMode = gameMode.endsWith("_online")

  if (isOnlineMode) {
    // For online modes, the game will be handled by the OnlineGameManager
    // This function should not be called directly for online games
    console.warn("renderPongPage called for online mode. Use OnlineGameManager instead.")
    return
  }
  ...
  if (isMobile) {
    ...
    if (gameMode === "2v1_local") {
      ...
      // Generic online mode controls display for desktop
      player1ControlsHtml = `
                <div class="${commonControlBoxClasses}">
                    <strong class="${strongPlayerClasses}" data-i18n="pong.player1">${getPlayerLabel(1)}</strong><br>
                    W - ${getMoveUp()}<br>
