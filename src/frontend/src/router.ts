...
const routes: { [key: string]: () => void } = {
  '/home': renderHomePage,
  '/': renderHomePage, // Mapea la raíz a la página de inicio
...
export async function navigateTo(path: string): Promise<void> {
  ...
  if (isAuthPage) {
    ...
    if (renderFunction) {
      renderFunction(); // Llama a la función de renderizado de login/register que maneja appRoot.innerHTML
    } else {
        console.warn(`Ruta no encontrada para página de autenticación: ${path}`);
    }
    // No renderizamos el navbar para las páginas de autenticación
  } else {
