       ...
       >
        ...
        >
          ...
          <p class="text-center text-gray-300 text-base mt-6">
            <span data-i18n-html="login.createAccountSentence">¿No tienes una cuenta? <a href="/register" id="create-account-link" class="text-[#ffc300] hover:underline font-semibold transition-colors duration-200 hover:text-[#ffd60a]" data-i18n="login.createAccountButton">Crea una cuenta</a></span>
          </p>
        </div>
        <style>
...
function applyTranslations(): void {
    ...
    const createAccountSentence = document.querySelector('[data-i18n-html="login.createAccountSentence"]');
    if (createAccountSentence) {
        const part1 = getTranslation('login', 'noAccountYet') || "¿No tienes una cuenta?"; // Fallback if key is missing
        const linkText = getTranslation('login', 'createAccountButton');
        createAccountSentence.innerHTML = `${part1} <a href="/register" id="create-account-link" class="text-[#ffc300] hover:underline font-semibold transition-colors duration-200 hover:text-[#ffd60a]">${linkText}</a>`;
    }

    const languageDisplaySpan = document.querySelector('#language-dropdown-button span[data-i18n="common.language"]');
    if (languageDisplaySpan) {
        languageDisplaySpan.textContent = getTranslation('common', 'language');
    }
}