...
fastify.post('/auth/google', async (request, reply) => {
  ...
  try {
    ...
    let user = userRes.ok ? (await userRes.json()) as DbService.User : null; //
    ...
    if (!user) {
      ...
      user = (await userRes.json()) as DbService.User;
    }
...
fastify.get('/auth/profile/stats', { preHandler: verifyToken }, async (request, reply) => {
  ...
  try {
    ...
    if (!res.ok) return reply.code(404).send({ message: 'Usuario no encontrado en el ranking' });
...
fastify.get('/auth/profile/download-historial', { preHandler: verifyToken }, async (request, reply) => {
  ...
  try {
    ...
    const matchHistory = (await res.json()) as Array<{
      game_id: number;
      result: string;
...
fastify.get('/auth/ranking', async (request, reply) => {
  try {
    ...
    const rankingTop100 = (await res.json()) as DbService.RankingEntry[];
...
fastify.get('/auth/games/live', async (request, reply) => {
  try {
    ...
    const liveMatches = (await res.json()) as DbService.GameLive[];
...
fastify.get('/auth/settings/user_data', { preHandler: verifyToken }, async (request, reply) => {
  ...
  const user = await getUserById(userId.toString());
  if (!user) return reply.code(404).send({ message: 'Usuario no encontrado' });
  return reply.send({ username: user.username, email: user.email });
});
...
fastify.get('/auth/settings/config', { preHandler: verifyToken }, async (request, reply) => {
  ...
  if (!profile) return reply.code(404).send({ message: 'Configuración no encontrada' });
  return reply.send({
